...
class RetinaFPN(nn.Module):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        for each_stride in strides:
            layers.append(block(self.in_planes, planes, each_stride))
    ...
    def _upsample_add(self, x, y):
        ...
        '''
        _, _, H, W = y.size()
        return functional.upsample(x, size=(H, W), mode='bilinear') + y

    def forward(self, x):
        # Bottom-up
        c1 = functional.relu(self.bn1(self.conv1(x)))
        c1 = functional.max_pool2d(c1, kernel_size=3, stride=2, padding=1)
        ...
        c5 = self.layer5(c4)
        p6 = self.conv6(c5)
        p7 = self.conv7(functional.relu(p6))
